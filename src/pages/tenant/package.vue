  ...
  <VPage class="p-24">
    ...
    <ze-table
      ...
      :columns="[
        ...
        { prop: 'packageName', label: '套餐名称', minWidth: 100, fixed: true },
...
const [listData, refresh, pagination, loading] = useTable(tenantPackageApi.listTenantPackage, searchForm, {
  immediate: true,
})
...
const isEdit = computed(() => editForm.value?.packageId || false)

const [menuOptions, fetchRoleMenutreeOpt] = useApi(
  menuApi.roleMenuTreeselect,
  computed(() => editForm.value.packageId || undefined),
  {
...
const [editForm, editFormItems, editFormRules] = useForm({
  ...
  packageName: {
    value: '',
    item: { type: 'text', label: '套餐名称', plh: '请输入角色名称' },
...
const { request: fetchEdit, loading: submitting } = useApi(
  (data: typeof editForm.value) =>
    isEdit.value ? tenantPackageApi.updateTenantPackage(data) : tenantPackageApi.addTenantPackage(data),
  editForm,
  {
...
const [deptOptions, fetchDeptTreeSelect] = useApi(tenantPackageApi.deptTreeSelect, '')
...
const [, handleDel] = useApi(tenantPackageApi.delTenantPackage, [], {
  onSuccess: () => refresh(),
  tipSuccess: '删除成功',
})
</script>
