  ...
  <VAsidePage>
    ...
    <ze-table
      ...
      :columns="[
        ...
        { prop: 'tenantId', label: '租户编号', minWidth: 138 },
        ...
        <template #default="{ row }">
          <ze-actions
            ...
            :actions="[
              ...
              { content: '删除', confirm: true, onClick: () => handleDel([row.userId]) },
            ]"
            ellipsis
          />
        </template>
...
import { tenantApi, userApi } from '@/api/_index'
...
const [tableData, refresh, pagination, loading] = useTable(tenantApi.listTenant, searchForm, {
  immediate: true,
})
...
const [editRef, EditModal] = useModal({
  title: computed(() => `${isEdit.value ? '编辑' : '添加'}`),
